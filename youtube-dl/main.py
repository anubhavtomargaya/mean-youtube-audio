...
@app.post("/ydl/api/v1/download")
def trigger_download(input:DownloadRequest): #2

    logger.logger.error('input: %s',input)
    try:
        url = input.url
        if settings.ACTIVE_PID !=0:
            try:
                ...
                return False
        title,pid = download(url,play=input.play)
        settings.ACTIVE_PID =pid
        logger.logger.info('respose" %s',title)
        return DownloadResponse(out_file=title,pid=pid)
